    ...
    <ParamField path="input.trail.compliance_status" type="object">
      ...
      <Expandable title="compliance_status properties">
        ...
        <ParamField path="input.trail.compliance_status.attestations_statuses" type="object">
          ...
          Some types put their summary under `attestation_data` rather than on the object itself. An `sbom` attestation keeps it at `attestation_data.document`, which holds `subject` and `package_count`.
        </ParamField>
