...
Once the first run completes (every two minutes by default), check the [Kosli app](https://app.kosli.com) under **Environments** — you will see environments named according to the template in `config.yaml`.
...
- **Private-only EKS endpoints require in-VPC deployment.** See the [EKS caveats](#eks-caveats).
