...
`config.yaml` at the repo root defines the policy Kosli Capture applies. It has three parts: **naming**, **scope**, and **types**. `make` validates and converts the file at build time, so a malformed config fails the deploy rather than shipping.
...
Resources matching **all** pairs of an entry are reported together in one snapshot call (`snapshot lambda --function-names a,b,c` / `snapshot ecs --clusters a,b,c`). Unmatched resources fall through to the default behavior — Lambda's `granularity`, ECS's per-cluster env — unchanged. If a resource matches more than one entry, the first entry wins; an entry with no tag pairs never matches anything.
...
EKS is the one resource type that may need extra setup on your side. Two things to be aware of:
