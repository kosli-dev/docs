...
<Info>
**This feature is in beta.** Controls, decisions, the `kosli attest decision` command, and the `for_control` policy requirement are all available in Kosli but in beta — the UI, CLI flags, and policy YAML schema may still change. If you have questions or comments, please reach out to the Kosli team.
</Info>
...
| Field | Description |
|-------|-------------|
| **Identifier** | **Required.** The control identifier (e.g. `RCTL-043`, `peer-review`, `vuln-scan-production`). Unique within your organisation and **immutable once created** — to use a different identifier, create a new control. |
...
| **Tags** | Optional. Key–value labels (e.g. `framework: SOC2`) for grouping and filtering controls. |

Controls can also be managed programmatically through the controls API (`POST /api/v2/controls/{org}`).

<Tip>
  Control identifiers are the stable identity that decisions and environment policies reference. Choose identifiers that match how your organisation already refers to controls — for example, the identifiers in your ServiceNow or GRC system. If you use `RCTL-043` today, use exactly that.
</Tip>
...
<Frame>
  <img src="/images/tutorials/controls-list.png" alt="Controls catalog listing controls with their identifier, description, tags, name, and current version, with search and tag-filter inputs" />
</Frame>
...
Navigate to **Controls** in the [Kosli app](https://app.kosli.com) and select a control to see its detail view. The header shows the identifier, current version, name, description, tags, who created it and when, and any links. Below it are two tabs: **Decisions** and **Versions**.

### Decisions

The Decisions tab is the default view. It lists every decision recorded against this control across all flows and trails, with the timestamp, outcome (compliant or non-compliant), the trail it was recorded on, and the artifact fingerprint. Filter by outcome and date range to narrow the list.
...
Select a decision to open its detail, showing the trail and artifact it applies to, the origin URL, any user-data, and attached evidence — such as the Rego policy and evaluation report behind the judgement.
