...
A Policy Enforcement Point (PEP) is where that decision is acted upon. `kosli assert control` is the PEP — it checks that a passing decision has been recorded for a named control on a trail, and exits non-zero if it hasn't. Place it at the point in your pipeline where you want to gate progress on control compliance.
...
A decision records the outcome of a PDP against a named control, scoped to a specific artifact. Use `--fingerprint` to identify the artifact the decision applies to — this is what allows `kosli assert control` and environment policies to later check compliance for that artifact specifically.
...
`kosli assert control` is the Policy Enforcement Point (PEP) for a named control. It checks that a passing decision has been recorded for a specific control on a trail, and exits non-zero if it hasn't. Use it as a pipeline gate at the point where you want to enforce control compliance — for example, before promoting an artifact to production.

```bash
kosli assert control \
    --control RCTL-043 \
    --flow my-release-flow \
    --trail my-release-trail \
    --fingerprint "$ARTIFACT_FINGERPRINT"
...
| Flag | Description |
|------|-------------|
| `--control` | **Required.** The control code to assert. |
| `--flow` | **Required.** The flow the trail belongs to. |
| `--trail` | **Required.** The trail to check for a passing decision. |
| `--fingerprint` | The SHA256 fingerprint of the artifact to check decisions for. Matches against artifact-scoped decisions recorded with the same fingerprint. |
| `--artifact-type` | The artifact type (e.g. `docker`, `file`). Provide this with the artifact name/path as the command argument instead of `--fingerprint` to have Kosli calculate the fingerprint. |

If a passing decision has been recorded for the control on the trail for the specified artifact, the command exits 0. If not, the command exits non-zero and prints the reason, failing the pipeline step.

<Tip>
  `kosli attest decision` is the PDP: it records a judgement about a control. `kosli assert control` is the PEP: it enforces that judgement. Keep them in separate pipeline steps — the PDP step is where evidence is evaluated and the outcome is recorded; the PEP step is where the pipeline gates on that recorded outcome.
</Tip>
...
sequenceDiagram
    ...
    rect rgb(255,235,215)
        ...
        Note over P: 🔵 PDP — evaluate RCTL-043
    ...
    rect rgb(220,255,220)
        ...
        Note over P: 🔴 PEP — assert artifact against environment policy
...
You have learned how to define controls in Kosli, record decisions against them from pipelines using `kosli attest decision` as the PDP, assert control compliance in your pipeline using `kosli assert control` as the PEP, enforce controls in environment policies, and view compliance across deployments.
