...
| Flag | Description |
|------|-------------|
| `--policy` | What to evaluate — a Kosli policy **identifier** (uses its current version) or a local `.rego` file. |
| `--control` | The control identifier the resulting decision is recorded against. |
| `--evaluation-context` | The facts to evaluate — the **source**. Defaults to the decision trail. |
| `--flow` / `--trail` | Where the decision is recorded — the **destination**. |

<Warning>
Read `--flow`/`--trail` and `--evaluation-context` carefully: the first pair says where the decision
...
| `--control` | The control identifier the resulting decision is recorded against. |
| `--evaluation-context` | One or more **typed references** to the facts to evaluate (the **source**), repeatable. In the beta the only reference *type* is a trail: `trail=<trail_id>` (in `--flow`), or `trail=<flow_id>/<trail_id>` to name a trail in another flow. Repeat the flag to evaluate several trails — see [Evaluating multiple trails](#evaluating-multiple-trails). Defaults to the decision trail. The typed form leaves room for other context types (snapshots, artifacts, …) later without new flags. |
...
| Recorded on the decision | Why it is there |
|---|---|
...
| The trail **moments** in scope, and the **attestation IDs** actually used | A policy may ignore half a trail, so the decision names the facts it actually relied on. |
...
them as `input.trails`. Repeat `--evaluation-context` to name each one:
...
kosli evaluate policy \
    ...
    --control RCTL-043 \
    --evaluation-context trail=build-ci/build-41 \
    --evaluation-context trail=build-ci/build-42 \
    --fingerprint "$ARTIFACT_FINGERPRINT" \
    --name pr-approval-decision \
    --sync
...
Sampling a decision usually raises the question of whether it was *right*. Because a decision
records everything it was given, you can export that and evaluate the policy again yourself, with
stock OPA, on your own machine:

```shell
kosli get decision DECISION-ID --output fixture --dir ./replay
```
