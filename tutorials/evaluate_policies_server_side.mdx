...
Pass a single `.rego` file, as in the example below, or a directory of them when a policy is split
into packages — Kosli stores the whole set as one versioned bundle with one digest, so a decision
still points at one unambiguous thing. Tests are stored with the bundle but never executed
server-side.
...
return the outcome, as the example below does.
