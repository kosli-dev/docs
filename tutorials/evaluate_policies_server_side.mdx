...
| `--trail` | The trail the decision is recorded on. Defaults to the single evaluation trail; required when evaluating multiple trails or a different flow. |
...
| `--evaluation-context` | The trail (or trails) whose facts are evaluated (the **source**). Defaults to the decision trail; combine with `--evaluation-flow` to evaluate trails in another flow. |
| `--evaluation-flow` | The flow that owns the trails in `--evaluation-context`, when the facts you evaluate live in a different flow than the decision is recorded on. Defaults to `--flow`. |
...
whose facts you evaluate. When the evidence lives in a *different* flow — for example, evaluating a
build flow's trail but recording the decision on a release flow's trail — set `--evaluation-flow`
for the source and `--flow` / `--trail` for the destination:

```shell
kosli evaluate policy \
    --policy pr-approved \
    --control RCTL-043 \
    --evaluation-flow build-ci \
    --evaluation-context build-42 \
    --flow release \
    --trail release-99 \
    --fingerprint "$ARTIFACT_FINGERPRINT" \
    --name pr-approval-decision \
