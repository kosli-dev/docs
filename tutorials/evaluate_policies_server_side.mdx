...
records a **decision** against a control.

Evaluation is **asynchronous by default** — the command returns once the evaluation is queued,
naming the **evaluation context** that will carry the outcome:
...
You then either poll that context (see
...
resolves these references into the [evaluation context](#check-an-asynchronous-evaluation) a decision cites;
...
queue. A decision is **valid as of the requested time**, even though it was recorded later. Kosli
records both and never backdates the record.
...
Without `--sync`, the outcome arrives on the **evaluation context** the command named. A context
exists from the moment the evaluation is queued, so you can ask about it before there is any
decision to look at:

```shell
kosli get evaluation-context ec_7f3a91c2
```
...
[When evaluation itself fails](#when-evaluation-itself-fails). Once it is `completed`, the context
names the decision it produced, so a queued evaluation and the decision it becomes are one hop apart
in both directions.
...
| Form | Returns |
|---|---|
| `kosli get evaluation-context ID` | The summary above. |
...
The typed error lands on the evaluation context, so a failure is visible without `--sync` or
...
```shell
kosli get evaluation-context CONTEXT-ID --fixture --dir ./replay
```
...
<Tip>
Because a context exists from the moment an evaluation is queued, this export also works when there
is **no decision to look at** — an evaluation still in the queue, or one that failed. That is usually
the case you most want to inspect.
</Tip>
