...
<Warning>
Read `--flow`/`--trail` and `--context` carefully: the first pair says where the decision
is **written**, the second says which facts are **read**. They are usually the same trail, and when
they differ it is deliberate — see *Evaluating facts from another flow*, below.
</Warning>
