...
When a policy denies, the decision records **why** through its `violations`. Whether the policy's
rules weren't met (for example, a pull request with no approver) or an expected attestation was
missing from the trail, both surface as violations on the decision.
