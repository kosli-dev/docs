...
### The whole loop in two commands

Publish a policy, then evaluate it. That is the feature:
...
`RCTL-043`, citing the policy version, the parameters, and the exact facts it used. `--assert` fails
the pipeline step if the policy denies. Everything else on this page is detail around those two
commands.
...
before you send feedback, so you don't spend it on something already deferred.
...
For **governance platform engineers**. The core loop, the Rego contract, testing, and managing
policies over time.
</Card>
...
human-readable **name**, a short **description** of its purpose, and an **input-context schema**
...
the policy, so it is **immutable once created**; choose one that matches how your organization
names its policies. `--name` and `--description` are the human-readable label and purpose, and
both can be changed later without changing the identifier.

Pass a single `.rego` file, as here, or a directory of them when a policy is split into packages —
Kosli stores the whole set as one versioned bundle with one digest, so a decision still points at
one unambiguous thing. Tests are stored with the bundle but never executed server-side.
...
| Flag | Description |
|------|-------------|
| `--policy` | The policy to run — a Kosli policy **identifier** (uses its current version) or a local `.rego` file. |
| `--control` | The control identifier the resulting decision is recorded against. |
| `--context` | What it runs against — the **source**. Defaults to the decision trail. |
| `--flow` / `--trail` | Where the decision is recorded — the **destination**. |

<Warning>
These flags pull in opposite directions, so it is worth reading them slowly. `--context` chooses
the trail whose facts are **read**. `--flow` and `--trail` choose the trail the decision is
**written** to. Most of the time all three name the same trail; when they don't, that is a
deliberate choice — see the *Evaluating facts from another flow* example below.
</Warning>
...
| `--control` | The control identifier the resulting decision is recorded against. |
| `--context` | One or more **typed references** to what should be evaluated (the **source**), repeatable. This is what Kosli resolves into the [evaluation context](#re-run-a-decision-offline) a decision cites — not a cluster or account selector. In the beta the only reference *type* is a trail: `trail=<trail_id>` (in `--flow`), or `trail=<flow_id>/<trail_id>` to name a trail in another flow. Repeat the flag to evaluate several trails — see [Evaluating multiple trails](#evaluating-multiple-trails). Defaults to the decision trail. The typed form leaves room for other context types — snapshots and reported workloads, for example — later, without new flags. |
...
whose facts you evaluate. Because each `--context` reference carries its own flow
...
A policy published with `--context-schema trails` evaluates several trails at once and receives
...
The context schema and the number of references have to agree: passing several references to a
`trail`-schema policy is rejected before the evaluation is queued, as is passing one reference to a
`trails`-schema policy. Because there is no single evaluation trail to default to, `--trail` is
required to say where the decision is recorded.
...
| Publish | Result |
|---|---|
| Same policy source, same name, description, and context schema | **No-op.** No version created. |
| Any change to the policy source | New version. The digest is over the bytes, so reformatting counts. |
| Same source, changed name, description, or context schema | New version — the description is part of what an auditor reads off a version. |
...
<Frame>
  <img src="/images/tutorials/policy-source.png" alt="Policy tab for a policy showing the bundle digest, roots, entrypoint and context schema, a file list, and the read-only Rego source of the selected file with line numbers" />
</Frame>
...
**context schema**, and the **parameters** the policy reads. Tests shipped with the policy are
...
| `policy.rego` | The exact policy source that ran, matching the digest on the decision. |
...
policy have said?" — is [out of scope for the beta](#whats-out-of-scope-for-the-beta). Exporting the
...
- **Verbose evaluation tracing** — beyond `violations` and any custom output the policy returns,
  there is no rule-by-rule trace of why a policy reached its result.
