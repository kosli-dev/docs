...
its **evaluation context** — the policy version that ran, the parameters used, and the trail
*moments* that were in scope (and, through them, the attestations that were evaluated).
...
the decision states exactly what was evaluated and when it applied. To review and filter every
decision a policy has produced, see
[View the decisions for a policy](#view-the-decisions-for-a-policy).
