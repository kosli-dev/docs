...
<Tip>
**Working with SARIF security scans?** The [`system:sarif` attestation type](/attestation_types/system/sarif) has a built-in one-step path that runs a rego policy and records the result in a single CLI call — no separate `kosli evaluate` + `kosli attest generic` orchestration needed. See [Attesting SARIF scans](/tutorials/attest_sarif) for the workflow. The general patterns in this tutorial still apply when you want to evaluate whole trails rather than individual scans.
</Tip>
...
<Info>
**One-step alternative for SARIF data.** The pattern above (run `kosli evaluate` against a file, extract `.allow`, attest with `kosli attest generic`) is the general-purpose route — it works for any input. For SARIF security scans specifically, the `system:sarif` attestation type collapses this into a single CLI call: `kosli attest system sarif --scan-results foo.sarif --policy foo.rego` runs the evaluation inline and records both the scan facts and the verdict on one attestation. See the [Attesting SARIF scans tutorial](/tutorials/attest_sarif) for the workflow.
</Info>
