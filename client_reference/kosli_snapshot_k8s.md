...
|    -h, --help  |  help for k8s  |
|    -k, --kubeconfig string  |  [defaulted] The kubeconfig path for the target cluster. (default "/Users/dangrondahl/.kube/config")  |
...
<AccordionGroup>
<Accordion title="report what is running in an entire cluster using kubeconfig at $HOME/.kube/config">
```shell
kosli snapshot k8s yourEnvironmentName 

```
</Accordion>
<Accordion title="report what is running in an entire cluster using kubeconfig at $HOME/.kube/config (with global flags defined in environment or in a config file)">
```shell
...
<Accordion title="report what is running in an entire cluster excluding some namespaces using kubeconfig at $HOME/.kube/config">
```shell
kosli snapshot k8s yourEnvironmentName 
...
<Accordion title="report what is running in a given namespace in the cluster using kubeconfig at $HOME/.kube/config">
```shell
kosli snapshot k8s yourEnvironmentName 
...
<Accordion title="report what is running in a cluster using kubeconfig at a custom path">
```shell
kosli snapshot k8s yourEnvironmentName 
