...
```shell
kosli attest sbom [IMAGE-NAME | FILE-PATH | DIR-PATH] [flags]
```
...
| `--sbom-file` | string | The path to the SBOM file. CycloneDX (JSON, XML) and SPDX (JSON, tag-value) are supported. |
...
<AccordionGroup>
<Accordion title="report an SBOM about a pre-built docker artifact (kosli finds the fingerprint)">
```shell
...
<Accordion title="report an SBOM about a trail">
```shell
kosli attest sbom 
	--name yourAttestationName 
	--sbom-file yourSbomPath 
