...
[Kosli Capture](https://github.com/kosli-dev/kosli-capture) reads its policy from a single `config.yaml` at the repo root. `make` validates and converts the file to `config.json` at build time (stdlib JSON at runtime — no PyYAML dependency), and a malformed file fails the conversion so bad config never ships.
...
- An entry with no tag pairs never matches anything, so a typo cannot sweep in everything.
