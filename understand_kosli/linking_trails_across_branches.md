...
Use `kosli get trail` on the main-branch build to query the PR trail's compliance status, then record the result as a custom attestation on the main trail.
...
<Steps>
  ...
  <Step title="Query the PR trail compliance">
    Use the Kosli CLI to retrieve the PR trail and check its compliance status:

    ```bash
    kosli get trail "$PR_TRAIL" \
      --flow app-pr \
      --output json
    ```

    Parse the JSON output to determine whether the trail is compliant. For example:
  ...
  <Step title="Attest the result on the main trail">
    Record a generic or custom attestation on your main-branch trail with the PR trail's compliance status:
      ...
      - name: Check PR trail compliance
        ...
        run: |
          ...
          kosli attest generic \
            ...
            --trail "$(git rev-parse HEAD)" \
            --compliant="$IS_COMPLIANT" \
            --description "PR trail ${{ steps.pr.outputs.trail }} compliance: ${IS_COMPLIANT}"
```
...
Until then, the `kosli get trail` approach described here is the recommended pattern.
