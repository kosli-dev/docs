...
for you to manage. Deploy the configuration once in each project that holds clusters you want Kosli
Capture to snapshot.

Kosli Capture runs in AWS and reaches your project through
[Workload Identity Federation](https://cloud.google.com/iam/docs/workload-identity-federation). Kosli
...
to your organization, which is the counterpart of the external ID. Every Kosli Capture job runs
...
If you wish to grant access for Kosli Capture without using our published configuration, the
...
<Note>
Kosli Capture connects to each cluster's Kubernetes API from Kosli's infrastructure, so the
...
variable "project_id" {
  type        = string
  description = "The GCP project that holds the GKE clusters Kosli Capture will snapshot."
}

variable "kosli_aws_account_id" {
  type        = string
  description = <<-EOT
    The AWS account in which Kosli Capture runs, supplied by Kosli. It differs
...
variable "kosli_role_name" {
  type        = string
  description = <<-EOT
    The name of the IAM role, in the Kosli AWS account, that Kosli Capture
...
# Kosli Capture runs in AWS. Workload Identity Federation lets it exchange its
...
resource "google_iam_workload_identity_pool_provider" "kosli_aws" {
  ...
  display_name                       = "Kosli Capture (AWS)"
  ...
  # Kosli-side role dedicated to your organization is accepted. A Kosli Capture
...
resource "google_service_account" "kosli_capture" {
  ...
  display_name = "Kosli Capture"
  description  = "Impersonated by Kosli Capture to snapshot GKE clusters. Managed by Infrastructure Manager."
}
...
output "kosli_capture_service_account" {
  description = "Email of the service account Kosli Capture impersonates; give this to Kosli."
...
# below when Kosli Capture calls the Kubernetes API, so this role is also the
...
resource "google_project_iam_custom_role" "kosli_capture" {
  ...
  title       = "Kosli Capture"
  description = "Read-only GKE inventory for Kosli Capture SDLC compliance evidence collection."
...
* A Kosli Capture worker picks up a job for your organization and assumes the role in your account
...
The trust policy on the role in your account limits access to the AWS account in which the Kosli
Capture Managed service is running. The ExternalId acts as a shared secret between Kosli and you, so that only
...
If the IAM role assumed by Kosli Capture is removed or edited within your cloud accounts, Kosli
