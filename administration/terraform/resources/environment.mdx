...
<Note>
For aggregating multiple physical environments into logical groups, use the [`kosli_logical_environment` resource](/administration/terraform/resources/logical_environment).
</Note>
...
For querying environment metadata such as `last_modified_at` and `last_reported_at` timestamps, use the [`kosli_environment` data source](/administration/terraform/data-sources/environment). This is useful for monitoring and creating conditional logic based on environment state.
