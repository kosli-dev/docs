...
To get started with SCIM provisioning, the team managing your SSO and IdP will need to configure the connection between your IdP and Kosli's auth provider. Different IdPs require different configuration, so Kosli's Customer Success team will provide you a link to the setup wizard with step-by-step instructions, which you can pass on to the relevant team.
...
<Frame><img src="/images/administration/scim-setup-wizard.png" alt="Start page for the SSO and SCIM setup suite wizard" /></Frame>
...
Kosli now allows handling role assignment through your IdP, so that users' roles within Kosli can be managed by your onboarding and IT teams, without requiring admin access to Kosli.
...
Note that once a user has been provisioned through SCIM, the IdP is authoritative and will override that user's pre-existing organization memberships/roles within Kosli.
...
<Frame><img src="/images/administration/scim-group-role-mapping.png" alt="Section in the SSO Setup Suite Wizard showing how to configure mapping of IdP groups to Kosli roles" /></Frame>
